...
{
    public event Action<GameObject, GameObject, GameObject> RoomCreated;
    ...
    [SerializeField] private Material _ceilMaterial;
    ...
    public void Build(ProjectData projectData)
    {
        ...
        BuildCeiling(projectData.Points);
        BuildBaseboards(projectData.Points);

        RoomCreated?.Invoke(_floorObject, _wallsRoot, _baseboardsRoot);
    }
    public void Clear()
    {
    ...
    private void BuildCeiling(Vector2[] points)
    {
        Mesh mesh = new();
        mesh.name = "Ceiling_Mesh";
        ...
        for (int i = 0; i < points.Length; i++)
        {
            Vector3 worldPoint = _config.BlueprintPointToWorld(points[i]);

            vertices[i] = new Vector3(worldPoint.x, height, worldPoint.z);
            uvs[i] = new Vector2(vertices[i].x, vertices[i].z);
        }

        int[] floorTriangles = Triangulate(points);
        int[] ceilingTriangles = new int[floorTriangles.Length];

        for (int i = 0; i < floorTriangles.Length; i += 3)
        {
            ceilingTriangles[i] = floorTriangles[i];
            ceilingTriangles[i + 1] = floorTriangles[i + 2];
            ceilingTriangles[i + 2] = floorTriangles[i + 1];
        }
        ...
        _ceilingObject = new GameObject("Ceiling");
        _ceilingObject.transform.SetParent(GetRoomRoot(), false);

        MeshFilter meshFilter = _ceilingObject.AddComponent<MeshFilter>();
        MeshRenderer meshRenderer = _ceilingObject.AddComponent<MeshRenderer>();
        MeshCollider meshCollider = _ceilingObject.AddComponent<MeshCollider>();

        meshFilter.sharedMesh = mesh;
        meshCollider.sharedMesh = mesh;

        meshRenderer.renderingLayerMask = 1u << 2;
        meshRenderer.material = _ceilMaterial;
    }
    ...
    private void BuildWallSegment(int index, Vector3 start, Vector3 end, float height)
    {
        ...
        Vector4 tangent = new(direction.x, direction.y, direction.z, 1f);
